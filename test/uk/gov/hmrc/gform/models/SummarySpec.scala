...
class SummarySpec extends Spec {
  ...
  trait Test extends ExampleData {
    ...
    val mockRepeatService = new RepeatingComponentService(null, null) {
      ...
      override def getAllFieldsInGroupForSummary(topFieldValue: FormComponent, groupField: Group, sessionCache: Option[CacheMap]): List[List[FormComponent]] = {
        List[List[FormComponent]]()
      }

      override def getAllFieldsInGroup(topFieldValue: FormComponent, groupField: Group, sessionCache:Option[CacheMap]): List[List[FormComponent]] = {
        List.empty[List[FormComponent]]
      }

      override def atomicFields(section: BaseSection, sessionCache:Option[CacheMap])(implicit hc: HeaderCarrier, ec: ExecutionContext): List[FormComponent] = {
        section.fields
      }
    }
  ...
  "Summary" should "display the summary sections" in new Test {
    val render = SummaryRenderingService.summaryForRender(f, rawDataFromBrowser, retrievals, formId, formTemplate, mockRepeatService, envelope, None, None)
  ...
  it should "display links to page sections" in new Test {
    ...
    val render = SummaryRenderingService.summaryForRender(f, Map(), retrievals, formId, formTemplate, mockRepeatService, envelope, None, None)
  ...
  it should "display the title when shortName is not present in the section" in new Test {
    val render = SummaryRenderingService.summaryForRender(f, Map.empty, retrievals, formId, formTemplate, mockRepeatService, envelope, None, None)
  ...
  it should "not render sections with includeIf expressions that evaluate to false" in new Test {
    ...
    val renderWithDataMatching = SummaryRenderingService.summaryForRender(f, Map(FormComponentId("firstName") -> Seq("Pete")), retrievals, formId, formTemplate, mockRepeatService, envelope, None, None)
    renderWithDataMatching.futureValue.snippets.size shouldBe 3
    val renderWithDataMismatch = SummaryRenderingService.summaryForRender(f, Map(FormComponentId("firstName") -> Seq("*Not*Pete")), retrievals, formId, formTemplate, mockRepeatService, envelope, None, None)
  ...
  it should "display Group Labels (or Group Short Names if specified)" in new Test {
    ...
    val render0 = SummaryRenderingService.summaryForRender(f, Map.empty[FormComponentId, Seq[String]], retrievals, formId, formTemplate, mockRepeatService, envelope, None, None)
    ...
    val render1 = SummaryRenderingService.summaryForRender(f, Map.empty[FormComponentId, Seq[String]], retrievals, formId, formTemplateWGroupWithShortname, mockRepeatService, envelope, None, None)
  ...
  "The Change hrefs" should "link to the correct page" in new Test {
    ...
    val summaryForRender = SummaryRenderingService.summaryForRender(f, Map(FormComponentId("firstName") -> Seq("*Not*Pete")), retrievals, formId, formTemplate, mockRepeatService, envelope, None, None)
